...
const datacrop = [
                {  
                  id: 1, 
                  address: "88 อาคารดีทัค ทาวเวอร์ 2 ยูนิต 1507 ชั้น 15 ถ.เทคโนโลยี <br/> แขวง บางเขน เขตบางเขน กรุงเทพมหานคร 10220",
    ...
    <div className="min-h-screen font-sans bg-slate-100">
      <header className="relative p-6 pt-12 text-white bg-blue-500">
        ...
        {/* ข้อมูลพนักงาน */}
          ...
          <p className="text-sm text-blue-100">{data.position}</p>
          <p className="mt-1 text-xs text-blue-200">{data.department}</p>
        </div>
      </header>
        ...
        <div className="p-4 mt-4 bg-white rounded-lg shadow-sm">
          <div className="flex">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">{`Call Center : ${datacrop.callcenter}`}</p>
              <p className="mt-1 text-sm text-slate-600">{`Email : ${datacrop.email}`}</p>
            </div>
          </div>
        </div>
